\documentclass[20pt, a1paper, portrait]{tikzposter}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{amsmath}
\graphicspath{{./images/}}
\usepackage[dvipsnames]{xcolor}
\usepackage{textgreek}
\definecolor{eurecomblue}{RGB}{0,159,227}
\definecolor{visuwalkorange}{RGB}{255,204,0}



% RAJOUTER TITRE DU PROJET, AUTEURS
% 30% TEXTE, 40% ILLU, 30% VIDE
% PRECISER CHALLENGE TECHNIQUES
% RESULTATS
% CONCLUSION
% AMELIORATIONS, REFERENCES



\definebackgroundstyle{eurecom}{
\node[inner sep=0pt] at (0,0){
    \includegraphics[width = \paperwidth,
    height = \paperheight]
    {poster-template.pdf}};
}

\defineblockstyle{bestblockstyle}{
titlewidthscale=0.9, bodywidthscale=1,titleleft,
titleoffsetx=0pt, titleoffsety=0pt, bodyoffsetx=0mm,
bodyverticalshift=10mm, roundedcorners=5,
titleinnersep=5mm, bodyinnersep=4mm,
bodyoffsety=8mm
}
{
\draw[color=eurecomblue, fill=blockbodybgcolor, line width=3pt,
rounded corners=\blockroundedcorners] (blockbody.south west)
rectangle (blockbody.north east);
\ifBlockHasTitle
\draw[color=eurecomblue, fill=eurecomblue
, line width=2pt,  
rounded corners=\blockroundedcorners] (blocktitle.south west)
rectangle (blocktitle.north east);
\fi
}

\usetitlestyle{Empty}

\usebackgroundstyle{eurecom}

\title {Visuwalk}

\titlegraphic{\includegraphics[width=0.20\paperwidth]{logo1.png}}
\author{\large Alexandre Avy, Hichem Khettab, Lou Marze, Hamza Parnica, Guillaume Ung}
\date{Semester 5}

\begin{document}

\useblockstyle{bestblockstyle}

\maketitle[titletotopverticalspace=90mm,width = 0.5\paperwidth]


\block[titleoffsety=30mm,bodyoffsety=38mm,bodyverticalshift=10mm,titleinnersep=5mm,bodyinnersep=4mm]{\textbf {Abstract}}{
\Large
The goal of this semester project is to guide ,with sounds, a visually impaired person to follow a line. \\
The available tools are a Rasperry Pi and its camera.
For this goal, one needs to : \\
Detect the line from the camera's video \\
Process the line with the right technique \\
Find a way to communicate instructions to the user
}

\begin{columns}

\column{0.5}

\block{
\textbf{Video / Image Processing}}{

\begin{center} \LARGE {Detecting the line}
\end{center}

The steps to detect the line from the video are the following :  \\
Converting the image to gray \\
Applying a low-pass filter \\
Applying a Canny edge detector \\
Applying a Hough Transform to retrieve the lines coordinate \\

\begin{center}
\includegraphics[height = 7cm, width = 6cm]{poster_images/reel3.jpg}
\includegraphics[height = 7cm, width = 6cm]{poster_images/gray.jpg}
\includegraphics[height = 7cm, width = 6cm]{poster_images/canny_edge.jpg}
\includegraphics[height = 7cm, width = 6cm]{poster_images/houghlines_maybe.jpg}
\\

\centering
\large omg ça marche  
\end{center}


\begin{center} \LARGE {Calculating the \textalpha\ and \textbeta\ angle}
\end{center}


\begin{center}
\includegraphics[height = 7cm, width = 6cm]{poster_images/angle_alpha1.png}
\includegraphics[height = 7cm, width = 6cm]{poster_images/angle_alpha.png}
\\

\centering
\large l'angle alpha wow
\end{center}

\begin{center}
\includegraphics[height = 7cm, width = 6cm]{poster_images/angle_beta_barycentre_targetpoint.png}
\includegraphics[height = 7cm, width = 6cm]{poster_images/angle_beta_barycentre_targetpoint.png}
\\

\centering
\large l'angle beta wow
\end{center}

\begin{center} \LARGE {Calculating the distance d}
\end{center}


}

\column{0.5}

\block{}{
\begin{center}
\includegraphics[height = 7cm, width = 6cm]{poster_images/distance_bas_barycentre.png}
\includegraphics[height = 7cm, width = 6cm]{poster_images/angle_beta_barycentre_targetpoint.png}
\\

\centering
\large la distance d wow
\end{center}

}

\block{}{
We choose \(\tau\) so that \(d_{95\%} = \frac{w}{2}\), thus we have
\[ 3\tau = \frac{w}{2}, \quad \tau = \frac{w}{6} \]
It follows,
\[f(d) = 1 - e^{\frac{-6d}{w}}\]
}

\block{\textbf{Sound Processing}}{}

\end{columns}

\end{document}
